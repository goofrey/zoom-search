...
CATALOG_PATH = ROOT / "specs/zoom-search/evaluation/golden-tests.md"
...
def _extract_catalog(markdown_text: str) -> list[dict[str, object]]:
    block_match = re.search(
        r"<!-- golden-test-catalog:start -->\s*```json\s*(.*?)\s*```\s*<!-- golden-test-catalog:end -->",
        markdown_text,
        re.DOTALL,
    )
    if not block_match:
        raise ValueError("golden test catalog block not found")
...
def main() -> int:
    print(
        "Historical showcase checker only validates archived peak-win reports. "
        "Use `python3 tools/check_regression_benchmarks.py` for CI-style regression gating."
    )
    ...
    if failures:
        print("\nHistorical showcase consistency check failed:", file=sys.stderr)
        for failure in failures:
            print(f"- {failure}", file=sys.stderr)
        return 1

    print("\nAll historical showcase cases passed.")
    return 0
